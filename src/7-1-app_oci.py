...
# ---------------------------------------------------------
# Load MCPClient from '7-1-client.py'
# ---------------------------------------------------------
current_dir = os.path.dirname(os.path.abspath(__file__))
client_module_path = os.path.join(current_dir, "7-1-client.py")
server_script_path = os.path.join(current_dir, "7-1-server.py")
...
def simple_genai_chat(client, model_id, compartment_id, system_instruction, user_message) -> str:
    """
    A simpler chat function that returns raw text, used for Sampling.
    Does NOT enforce JSON schema.
    """
    chat_request = None
    if "cohere" in model_id.lower():
        chat_details = oci.generative_ai_inference.models.CohereChatRequest(
            ...
            preamble_override=system_instruction, temperature=0.0, max_tokens=1000
        )
    else:
        chat_details = oci.generative_ai_inference.models.GenericChatRequest(
            messages=[
                oci.generative_ai_inference.models.Message(role="SYSTEM", content=[oci.generative_ai_inference.models.TextContent(text=system_instruction)]),
    ...
    try:
        ...
        if "cohere" in model_id.lower():
            ...
            return "Generic model response not fully implemented"
    except Exception as e:
        return f"Error calling OCI: {e}"
...
def call_oci_genai(client, model_id: str, compartment_id: str, system_instruction: str, user_message: str) -> AgentDecision:
    ...
    # Re-use simple chat for the call, then parse
    response_text = simple_genai_chat(client, model_id, compartment_id, system_prompt_with_schema, user_message)
    ...
    try:
        ...
        print(f"[OCI Error] Failed to parse JSON: {e}")
        # Fallback decision
        return AgentDecision(
            thought="Failed to parse model response, asking user to retry.",
            use_tool=False,
            final_response="Sorry, I encountered an internal error processing the AI response."
        )
...
def main():
    ...
    def notification_handler(data):
        ...
        if method == "notifications/message":
             ...
             print(f"[Details] {message_data}")
        else:
             print(f"[Notification] {method}: {params}")

    def sampling_handler(messages, system_prompt=None):
        """
        Handle 'sampling/createMessage' request from server.
        Receives a list of messages (MCP format), returns a string response.
        """
        print(f"\n[Sampling] Server requested text generation...")
        
        # Extract the user message from the list (simplified: take last user message)
        user_text = ""
        system_text = system_prompt if system_prompt else "You are a helpful assistant." # Use provided system prompt or default
        ...
        for msg in messages:
            ...
            if role == "user":
                user_text = text
            # elif role == "system": # System prompt is now passed separately
            #    system_text = text

        if not user_text:
            return "Error: No user message found in sampling request."

        # Call OCI GenAI (Raw Text Mode)
        result_text = simple_genai_chat(genai_client, model_id, department_id, system_text, user_text)
        print(f"[Sampling] Generated: {result_text[:50]}...")
        return result_text
    ...
    try:
        ...
        try:
            ...
            # Using 'math_tutor' just to verify prompts still work, 
            # though we are focusing on shell command generation.
            ...
            if has_planner:
                ...
                if messages and messages[0].get("content", {}).get("type") == "text":
                    ...
                    print(f"[Setup] Loaded system prompt: {target_prompt_name}")
        except Exception as e:
            print(f"[Warn] Failed to fetch prompts: {e}")

        # 6. Chat Loop
        print("\n--- OCI GenAI Agent Started (Sampling Enabled) ---")
